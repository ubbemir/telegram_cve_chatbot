...
pub async fn cvss_chart(arg: &str) -> Result<String, Box<dyn Error + Send>> {
    ...
    let mut cvss_count = vec![("Low", 0u64), ("Medium", 0u64), ("High", 0u64)];
    for item in response.vulnerabilities {
        if let Some(metrics) = item.cve.metrics.cvssMetricV2 {
            match metrics[0].baseSeverity.to_lowercase().as_str() {
